...
function aef_demo_upsert_page( $slug, $title_vi, $title_en, $excerpt_vi, $content_vi, $content_en = '', $excerpt_en = '' ) {
	...
	if ( $id && ! is_wp_error( $id ) ) {
		...
		update_post_meta( $id, 'excerpt_vi', $excerpt_vi );
		update_post_meta( $id, 'content_en', $content_en ? $content_en : $content_vi );
		if ( $excerpt_en ) {
			update_post_meta( $id, 'excerpt_en', $excerpt_en );
		}
	}
	return $id;
}
...
function aef_apply_contentweb_copy() {
	...
	$copy['home_hero_cta2_en'] = 'Register';
	$copy['home_hero_cta2_vi'] = 'Đăng ký tham dự';
	...
	foreach ( array(
		...
		'high-level' => array(
			'Buổi sáng: phiên toàn thể cấp cao (truyền hình trực tiếp) và đối thoại cùng Thủ tướng. Buổi chiều: tham luận chuyên sâu và đối thoại cấp Bộ, có Phó Thủ tướng Chính phủ.',
			'Morning: high-level plenary (live) and dialogue with the Prime Minister. Afternoon: in-depth presentations and ministerial dialogue, with the Deputy Prime Minister.',
		),
	) as $slug => $notes ) {
